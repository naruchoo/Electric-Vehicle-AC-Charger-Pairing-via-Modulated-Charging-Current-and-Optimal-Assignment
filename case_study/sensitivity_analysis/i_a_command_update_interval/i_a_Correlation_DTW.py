...
out_dir = os.path.join(
    os.getcwd(),
    "case_study/sensitivity_analysis/i_a_command_update_interval/accuracy/Correlation_DTW"
)
    ...
    for ev_interval in ev_intervals:
        for tau in taus:
            ...
            for offset in range(30):
                ...
                # Compute the Correlation-DTW score matrix (similarity / distance metric)
                df_score = calculate_correlation_dtw(df_charger, df_ev)

                # Perform pair-identification (matching) and evaluate pairing (matching) accuracy
                _, acc = perform_matching_correlation_dtw(df_score)
            ...
            fname = f"{num_patterns}p_evint{ev_interval}_tau{tau}_CorDTWAcc.csv"
